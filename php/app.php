...
function db(): PDO {
    ...
    $db->exec("DELETE FROM settings WHERE key='admin_hash'");
    return $db;
}
...
function refresh_session_authorization(): void {
    ...
    if(empty($_SESSION['user_id'])){$_SESSION=[];session_destroy();return;}
...
function initial_administrator_required(): bool {
    return (int)db()->query("SELECT COUNT(*) FROM users u JOIN role_permissions rp ON rp.role_id=u.role_id AND rp.permission='Administrator' WHERE u.enabled=1")->fetchColumn()===0;
}

function eligible_site_support_user(int $userId): array|false {
    $q=db()->prepare('SELECT id,username,full_name,email,enabled FROM users WHERE id=? LIMIT 1');$q->execute([$userId]);$user=$q->fetch(PDO::FETCH_ASSOC);
    return $user&&$user['enabled']&&filter_var($user['email'],FILTER_VALIDATE_EMAIL)?$user:false;
}
...
function page_start(string $title): void {
    echo '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'.h($title).'</title><link rel="icon" href="/assets/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/semantic-ui-css@2.5.0/semantic.min.css"><link rel="stylesheet" href="/assets/style.css"></head><body class="notifyrouter"><header class="ui borderless menu"><a href="/" class="header item brand-link"><img class="brand" src="/assets/logo.png" alt="NotifyRouter"></a>';
    if(!empty($_SESSION['admin']))echo'<nav class="right menu header-nav"><a class="item" href="/">Rules &amp; Outputs</a><a class="item settings-link" href="/admin/settings"><img class="nav-avatar" src="'.h(profile_url()).'" alt=""><span>Settings</span></a>'.(can_access_administration()?'<a class="item" href="/admin">Administration</a>':'').'<div class="item"><form method="post" action="/"><input type="hidden" name="csrf" value="'.h(csrf()).'"><input type="hidden" name="action" value="logout"><button class="ui secondary button">Sign Out</button></form></div></nav>';
    echo'</header><main class="ui container">';
}
function page_end(): void {
    echo '</main><footer class="ui inverted vertical segment"><div class="ui container"><span><a href="'.NOTIFYROUTER_GITHUB.'" target="_blank" rel="noopener noreferrer">NotifyRouter</a> <a href="'.NOTIFYROUTER_RELEASE.'" target="_blank" rel="noopener noreferrer">v'.NOTIFYROUTER_VERSION.'</a></span><span><a href="'.NOTIFYROUTER_AUTHOR.'" target="_blank" rel="noopener noreferrer">Terry Rogers</a> © '.date('Y').'</span></div></footer><script>document.querySelectorAll("form:not(.inline)").forEach(e=>e.classList.add("ui","form"));document.querySelectorAll(".card").forEach(e=>e.classList.add("ui","segment"));document.querySelectorAll("table").forEach(e=>e.classList.add("ui","celled","compact","table"));document.querySelectorAll("button:not(.ui),a.button").forEach(e=>e.classList.add("ui","button"));document.querySelectorAll("button.danger").forEach(e=>e.classList.add("red"));document.querySelectorAll(".notice").forEach(e=>e.classList.add("ui","info","message"));document.querySelectorAll(".indicator,.status").forEach(e=>e.classList.add("ui","label"));</script></body></html>';
}
function require_admin(): void {session_start_secure();if(empty($_SESSION['admin'])){header('Location:/');exit;}}
...
session_start_secure();refresh_session_authorization();$setup=initial_administrator_required();
if($path==='/admin/setup'){
    $ok=$setup&&hash_equals(cfg()['setup_token'],(string)($_GET['token']??$_POST['token']??''));$error='';
    if($_SERVER['REQUEST_METHOD']==='POST'&&$ok){$username=trim((string)($_POST['username']??''));$fullName=trim((string)($_POST['full_name']??''));$email=trim((string)($_POST['email']??''));$password=(string)($_POST['password']??'');if(!preg_match('/^[A-Za-z0-9_.-]{3,64}$/',$username)||$fullName===''||!filter_var($email,FILTER_VALIDATE_EMAIL)||strlen($password)<12)$error='Enter a valid username, full name, email address, and password of at least 12 characters.';else{$roleId=(int)db()->query("SELECT id FROM roles WHERE name='Administrator'")->fetchColumn();$now=gmdate(DATE_ATOM);try{db()->prepare('INSERT INTO users(username,full_name,email,password_hash,role_id,enabled,created_at,updated_at)VALUES(?,?,?,?,?,1,?,?)')->execute([$username,$fullName,$email,password_hash($password,PASSWORD_DEFAULT),$roleId,$now,$now]);audit('Configuration Change','Initial administrator user created: '.$username,'Setup');header('Location:/');exit;}catch(PDOException){$error='That username or email address is already in use.';}}}
    page_start('Admin Setup');echo'<section class="card narrow"><h1>Admin Setup</h1>';if(!$setup)echo'<p>Setup is complete. <a href="/">Sign In</a>.</p>';elseif(!$ok)echo'<p class="error">A valid one-time setup link is required.</p>';else echo($error?'<p class="error">'.h($error).'</p>':'').'<p>Create the first enabled administrator user. This setup link is disabled afterwards.</p><form method="post"><input type="hidden" name="token" value="'.h(cfg()['setup_token']).'"><label>Username<input name="username" pattern="[A-Za-z0-9_.-]{3,64}" required autofocus></label><label>Full Name<input name="full_name" required></label><label>Email Address<input type="email" name="email" required></label><label>Password<input type="password" name="password" minlength="12" required></label><button>Complete Setup</button></form>';echo'</section>';page_end();exit;
}
...
if($path==='/admin/reset'){
    ...
    if($_SERVER['REQUEST_METHOD']==='POST'&&$id){$password=(string)($_POST['password']??'');if(strlen($password)<12)$error='Use at least 12 characters.';else{db()->prepare('UPDATE users SET password_hash=?,updated_at=? WHERE id=?')->execute([password_hash($password,PASSWORD_DEFAULT),gmdate(DATE_ATOM),(int)$reset['user_id']]);db()->prepare('UPDATE reset_tokens SET used_at=? WHERE id=?')->execute([time(),$id]);audit('Password Change','Password changed through a reset link','Anonymous');session_destroy();header('Location:/');exit;}}
...
if(empty($_SESSION['admin'])){
    $error='';if($_SERVER['REQUEST_METHOD']==='POST'){$username=trim((string)($_POST['username']??''));$password=(string)($_POST['password']??'');$q=db()->prepare('SELECT u.*,r.name role_name FROM users u JOIN roles r ON r.id=u.role_id WHERE lower(u.username)=lower(?) LIMIT 1');$q->execute([$username]);$user=$q->fetch(PDO::FETCH_ASSOC);$valid=$user&&$user['enabled']&&password_verify($password,$user['password_hash']);$needsEnrollment=false;if($valid&&setting('enforce_2fa')==='1'){if(empty($user['two_factor_secret']))$needsEnrollment=true;else$valid=totp_valid(dec((string)$user['two_factor_secret']),(string)($_POST['otp']??''));}if($valid){session_regenerate_id(true);$_SESSION['admin']=true;$_SESSION['csrf']=bin2hex(random_bytes(24));$_SESSION['user_id']=(int)$user['id'];$_SESSION['user_name']=$user['full_name'];$_SESSION['permissions']=role_permissions((int)$user['role_id']);if($needsEnrollment)$_SESSION['must_register_2fa']=true;audit('Login','Successful sign in');header('Location:'.($needsEnrollment?'/admin/settings':'/'));exit;}$error='Incorrect username, password, or verification code.';audit('Login','Failed sign in for username '.$username,'Anonymous');}
    page_start('Admin Sign In');echo'<section class="card narrow"><h1>Admin Sign In</h1>'.($error?'<p class="error">'.h($error).'</p>':'').'<form method="post"><label>Username<input name="username" required autofocus autocomplete="username"></label><label>Password<input type="password" name="password" required autocomplete="current-password"></label><label>2FA Verification Code <span>Required when 2FA enforcement is enabled.</span><input name="otp" inputmode="numeric" pattern="[0-9]{6}" autocomplete="one-time-code"></label><button>Sign In</button></form><p><a href="/admin/forgot">Forgot Password?</a></p></section>';page_end();exit;
}
if(!empty($_SESSION['must_register_2fa'])&&$path!=='/admin/settings'){
    if($_SERVER['REQUEST_METHOD']==='POST'&&($_POST['action']??'')==='logout'){require_csrf();audit('Logout','Successful sign out');session_destroy();header('Location:/');exit;}
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($action==='logout'){audit('Logout','Successful sign out');session_destroy();header('Location:/');exit;}
    ...
    if($action==='profile'){$email=trim((string)($_POST['profile_email']??''));setv('profile_email',$email);$newPassword=(string)($_POST['new_password']??'');if($newPassword!==''&&strlen($newPassword)<12)$message='Password must be at least 12 characters.';else{$params=[$email,gmdate(DATE_ATOM)];$sql='UPDATE users SET email=?,updated_at=?';if($newPassword!==''){$sql.=',password_hash=?';$params[]=password_hash($newPassword,PASSWORD_DEFAULT);}$sql.=' WHERE id=?';$params[]=(int)$_SESSION['user_id'];db()->prepare($sql)->execute($params);audit($newPassword!==''?'Password Change':'Configuration Change',$newPassword!==''?'Profile and password updated':'Profile updated');$message=$newPassword!==''?'Profile and password updated.':'Profile updated.';}if(isset($_FILES['profile_picture'])&&$_FILES['profile_picture']['error']===UPLOAD_ERR_OK&&$_FILES['profile_picture']['size']<=2097152){$mime=(new finfo(FILEINFO_MIME_TYPE))->file($_FILES['profile_picture']['tmp_name']);$ext=['image/jpeg'=>'jpg','image/png'=>'png','image/webp'=>'webp'][$mime]??'';if($ext!==''){$relative='profile.'.$ext;$target=dirname(cfg()['database']).'/'.$relative;if(move_uploaded_file($_FILES['profile_picture']['tmp_name'],$target)){chmod($target,0640);setv('profile_picture',$relative);}}}}
    ...
    if($action==='save_user'){
        ...
        $siteSupportId=(int)setting('site_support_user_id');
        if(!preg_match('/^[A-Za-z0-9_.-]{3,64}$/',$username)||$fullName===''||!filter_var($email,FILTER_VALIDATE_EMAIL)||$roleId<1){$message='Enter a valid username, full name, email address, and role.';}elseif($siteSupportId>0&&$id===$siteSupportId&&!$enabled){$message='Choose a different Site Support User before disabling this account.';}else{
    ...
    if($action==='delete_user'){require_permission('Administrator');$id=(int)$_POST['id'];if($id===(int)($_SESSION['user_id']??0))$message='You cannot delete your own account.';elseif($id===(int)setting('site_support_user_id'))$message='Choose a different Site Support User before deleting this account.';else{db()->prepare('DELETE FROM users WHERE id=?')->execute([$id]);audit('Configuration Change','User deleted: ID '.$id);$message='User deleted.';}}
    ...
    if($action==='general'){require_permission('Administrator');$supportId=(int)($_POST['site_support_user_id']??0);$supportUser=eligible_site_support_user($supportId);$enable2fa=isset($_POST['enforce_2fa']);$unenrolled=(int)db()->query("SELECT COUNT(*) FROM users WHERE enabled=1 AND (two_factor_secret IS NULL OR two_factor_secret='')")->fetchColumn();$registeredAdministrators=(int)db()->query("SELECT COUNT(*) FROM users u JOIN role_permissions rp ON rp.role_id=u.role_id AND rp.permission='Administrator' WHERE u.enabled=1 AND u.two_factor_secret IS NOT NULL AND u.two_factor_secret<>''")->fetchColumn();if(!$supportUser)$message='Select an enabled Site Support User with a valid email address.';elseif($enable2fa&&($unenrolled||$registeredAdministrators<1))$message='2FA cannot be enforced until every enabled user is registered and at least one enabled administrator has registered 2FA.';else{setv('site_support_user_id',(string)$supportId);setv('enforce_2fa',$enable2fa?'1':'0');audit('Configuration Change','General settings updated; Site Support User ID '.$supportId.'; 2FA enforcement '.($enable2fa?'enabled':'disabled'));$message='General settings saved.';}}
...
$adminSection=trim((string)($_GET['section']??''))?:'overview';
if($path==='/admin'){
    require_permission('Read-only Administrator');
    $section=$adminSection;
    $allowedSections=['overview','general','users','roles','service-health','destinations','email','log'];
...
page_start($path==='/admin/settings'?'Settings':'Rules & Outputs');
?>
