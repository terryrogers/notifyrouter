...
const NOTIFYROUTER_VERSION = '0.3.0';
...
function send_email(string $to,string $subject,string $html): string {
    ...
    if(!smtp_command($s,'',[220])||!smtp_command($s,'EHLO notifyrouter.example.com',[250])){fclose($s);return'failed:greeting';}
    if($security==='starttls'){if(!smtp_command($s,'STARTTLS',[220])||!stream_socket_enable_crypto($s,true,STREAM_CRYPTO_METHOD_TLS_CLIENT)||!smtp_command($s,'EHLO notifyrouter.example.com',[250])){fclose($s);return'failed:tls';}}
...
function log_event(array $entry): void {
    $path=cfg()['event_log']??'/srv/www/.notifyrouter/notifyrouter-events.ndjson';
...
function page_start(string $title): void {
    echo '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'.h($title).'</title><link rel="stylesheet" href="/assets/style.css"></head><body><header><img class="brand" src="/assets/logo.png" alt="NotifyRouter"></header><main>';
}
function page_end(): void {
    echo '</main><footer><a href="'.NOTIFYROUTER_GITHUB.'" target="_blank" rel="noopener">NotifyRouter v'.NOTIFYROUTER_VERSION.'</a><span>© '.date('Y').' <a href="'.NOTIFYROUTER_AUTHOR.'" target="_blank" rel="noopener">Terry Rogers</a></span></footer></body></html>';
}
...
if($path==='/admin/forgot'){
    session_start_secure();$notice='';if($_SERVER['REQUEST_METHOD']==='POST'){$last=(int)($_SESSION['reset_requested_at']??0);if(time()-$last>=60){$_SESSION['reset_requested_at']=time();$channel=$_POST['channel']??'email';$token=bin2hex(random_bytes(32));$q=db()->prepare('INSERT INTO reset_tokens(token_hash,expires_at)VALUES(?,?)');$q->execute([hash('sha256',$token),time()+1800]);$scheme=((!empty($_SERVER['HTTPS'])&&$_SERVER['HTTPS']!=='off')||strtolower((string)($_SERVER['HTTP_X_FORWARDED_PROTO']??''))==='https')?'https':'http';$link=$scheme.'://notifyrouter.example.com/admin/reset?token='.$token;$message='<b>Password reset requested.</b><br><a href="'.$link.'">Reset Password</a><br>This link expires in 30 minutes.';$result=$channel==='pushover'?send_pushover(['title'=>'NotifyRouter Password Reset','message'=>$message,'html'=>true,'priority'=>0,'sound'=>'pushover']):send_email(setting('profile_email'),'NotifyRouter Password Reset',$message);if($result!=='sent')db()->prepare('DELETE FROM reset_tokens WHERE token_hash=?')->execute([hash('sha256',$token)]);}$notice='If that recovery channel is configured and available, a reset link has been sent.';}
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($action==='test_pushover')$message='Pushover test: '.send_pushover(['title'=>'NotifyRouter Test','message'=>'<b>Test successful.</b> Pushover settings are working.','html'=>true,'priority'=>0,'sound'=>'pushover']);
    if($action==='test_email')$message='Email test: '.send_email(setting('profile_email'),'NotifyRouter Test','<b>Test successful.</b> Email settings are working.');
    if($action==='clear_events'){db()->exec('DELETE FROM events');$message='Recent events cleared.';}
    if($action==='clear_log'){$log=cfg()['event_log']??'/srv/www/.notifyrouter/notifyrouter-events.ndjson';file_put_contents($log,'',LOCK_EX);$message='Captured payload log cleared.';}
